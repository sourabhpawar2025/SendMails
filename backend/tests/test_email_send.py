"""Test script: verify email sending flow (Django, single recipient from results)."""
import os
import sys
...
def main():
    try:
        ...
        count = Result.objects.exclude(email__isnull=True).exclude(email="").count()
        print(f"[OK] Recipients in results (email not null): {count}")

        senders = list(SmtpSender.objects.filter(is_active=True))
        if not senders:
        ...
        one = Result.objects.exclude(email__isnull=True).exclude(email="").first()
        if not one:
